...
6. **Fase B.2 — Janela por extremos, mapeamento `top10` e coerência com legado de export** (ver [ADR 0008](adrs/0008-descoberta-superficie-mcp-e-mapeamento-legado-top10-v1.md) e a [Fase 23 do guia](spec-driven-execution-guide.md)). **Não** usar [ADR 0007](adrs/0007-agregados-canonicos-de-janela-v1.md) como fonte de descoberta (D1) ou de mapeamento Top 10 — 0007 cobre `summarize_window_aggregates` (Fase B.1).
   ...
   - **Equivalência (nome novo autoexplicativo):** na mesma janela/fixture, `total_de_presencas_na_janela_por_dezena@1.0.0` deve ser **idêntico** a `frequencia_por_dezena@1.0.0` (comparação 1:1 do vetor), para permitir migração sem mudança de semântica.
   - **Sequência atual (objetivo principal):** acrescentar golden/teste de fórmula para `sequencia_atual_de_presencas_por_dezena@1.0.0` provando **reinício ao ausente** (streak atual no fim do recorte).
   - **Sunset (compatibilidade com prazo):** após a data de sunset definida no catálogo (ex.: `2026-05-25`), requests que usem um **nome deprecated** podem falhar com `UNKNOWN_METRIC` e `details.replacement_metric_name` + `details.sunset_date` (ver contrato).
...
| Nome novo (equivalência) | `total_de_presencas_na_janela_por_dezena@1.0.0` retorna exatamente o mesmo vetor que `frequencia_por_dezena@1.0.0` na mesma janela/fixture (teste de migração semântica). | [metric-catalog.md](metric-catalog.md) (Tabela 2 + nota editorial); ADR 0021 apêndice (texto humano) |
| Sequência atual (streak) | `sequencia_atual_de_presencas_por_dezena@1.0.0` prova o reinício ao ausente com fixture sintética pequena (assert por dezena e caso “ausente no último concurso ⇒ 0”). | [metric-catalog.md](metric-catalog.md) Tabela 2 |
| Sunset de nome deprecated | Após a data de sunset, pedir o nome deprecated retorna `UNKNOWN_METRIC` com `details.replacement_metric_name` e `details.sunset_date` (ISO). | [mcp-tool-contract.md](mcp-tool-contract.md) (`UNKNOWN_METRIC`/details); política no catálogo |
