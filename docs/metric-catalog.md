...
| `total_de_presencas_na_janela_por_dezena` | Total de presenças de cada dezena na janela (nome autoexplicativo; **equivalente** a `frequencia_por_dezena` **na mesma janela**). | `total[d] = contagem(d em concursos da janela)` (**equivalente** a `frequencia_por_dezena`); soma dos 25 = `15 × window_size`. | Histórico | MCP / composição / geração |
| `sequencia_atual_de_presencas_por_dezena` | Sequência atual (streak) de presenças por dezena ao fim da janela (**reinicia ao ausente**). | Percorrer concursos em ordem; para cada dezena `d`: se `d ∈ J_t` então `c[d] = c[d] + 1`, senão `c[d] = 0`. Retornar `c[d]` no fim do recorte. | Histórico | MCP / composição |
...
- **Onboarding e discovery não substituem o catálogo:** textos curtos em `help`, `getting-started@1.0.0` e `discover_capabilities` podem resumir nomes, exemplos iniciais e constraints operacionais, mas **não** redefinem fórmula, shape, unidade, versão ou semântica das métricas; a fonte normativa continua sendo este catálogo em conjunto com o contrato MCP.
