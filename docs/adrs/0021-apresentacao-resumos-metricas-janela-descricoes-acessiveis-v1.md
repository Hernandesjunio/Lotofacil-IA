...
**Nota (vocabulário «ausência»):** no discurso informal, «ausente» muitas vezes significa *faz N concursos que não saiu* — padrão numérico e semântica do **`atraso_por_dezena`** (e, no fim do recorte, muito do que se resume com **`estado_atual_dezena`**). A métrica canónica **`ausencia_blocos`** (listas de comprimentos de blocos consecutivos *sem* a dezena, [metric-catalog Tabela 2](../metric-catalog.md#tabela-2--semântica)) **não** é o mesmo *shape* de um vector 1..25; não confundir os três nomes. Alinhamento de fundo: [metric-glossary, âncora *Vocabulário* (ADR)](../metric-glossary.md#vocab-ausencia-adr-0021) e a secção **Rótulo de export legado** em [metric-catalog.md](../metric-catalog.md#rótulo-de-export-legado-qtdfrequencia-vector-1..25-em-gráfico).
