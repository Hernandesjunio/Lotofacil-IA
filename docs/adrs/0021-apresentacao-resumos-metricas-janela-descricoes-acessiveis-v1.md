...
- A **redação canónica** para copiar/ajustar fica no **[metric-glossary.md](../metric-glossary.md)** (já com `estabilidade_ranking` e demais entradas). A secção **[Vocabulário: «ausência» × frequência × atraso × `ausencia_blocos`](../metric-glossary.md#vocab-ausencia-adr-0021)** liga, sem alterar o [metric-catalog.md](../metric-catalog.md), a conversa *«ausência»* (frio / N concursos) aos nomes de métrica; o **QtdFrequencia** continua a mapear **só** `frequencia_por_dezena` (contagens).
- **Quatro *papéis*** (a distinguir *sem* renomear métricas, em linha com o [catálogo, secção *QtdFrequencia*](../metric-catalog.md#export-legado-qtdfrequencia) e a tabela *Ponte* homónima nessa secção): **(1)** *frequência na janela* (`QtdFrequencia` / `frequencia_por_dezena`); **(2)** atraso e leitura *agora* (`atraso_por_dezena`, `estado_atual_dezena`); **(3)** comprimentos de blocos *sem* a dezena (`ausencia_blocos`); **(4)** *distinção em tabela* (quadro *Vocabulário* + tabela A / nota *Ausência* nesta ADR) para o mesmo léxico *não* trocar rótulos. Fórmulas: [**Tabela 1**](../metric-catalog.md#tabela-1-identificacao-e-tipagem) e [**Tabela 2**](../metric-catalog.md#tabela-2-semantica) do catálogo, não reescritas aqui.
- **Hotfix editorial (V1 nova, sem quebrar legado):** esta ADR passa a também referenciar duas métricas **autoexplicativas** novas para painel humano, que distinguem *contagem total* de *sequência atual* (reinício ao quebrar). As métricas antigas permanecem disponíveis durante a janela de migração e, após o sunset, podem falhar com `UNKNOWN_METRIC` com `details` apontando substituição (política documentada no contrato e no catálogo, não nesta ADR).
- Esta ADR inclui um **Apêndice** com **frases modelo** (PT) para tabelas A e B, em eco com o glossário; extensões futuras mantêm a âncora `vocab-ausencia-adr-0021` (ver *Consequências*).
...
- [x] Subsecção de *Vocabulário* (âncora `vocab-ausencia-adr-0021`) e ligação ao Apêndice/ catálogo — feita em 2026-04-24; ainda aplica tarefas 27.1+ no [guia de fases](../fases-execucao-templates.md#fase-27---adr-0021-apresentacao-de-resumos-de-janela-tabelas-a-b-glossario-d5) se for preciso replicar texto a resources/ help.
...
<a id="apendice-frases-modelo-pt-adr-0021"></a>
...
| `frequencia_por_dezena` | Número de vezes em que cada dezena (1 a 25) **saiu** nos concursos **desta janela** (cada concurso conta no máximo uma vez por dezena). A **soma** dos 25 valores é `15 × N` concursos (N = tamanho da janela). **Não** descreve «há quantos concursos a dezena *não* saiu» — isso é atraso (ver linha seguinte e a nota *Ausência* abaixo). |
| `total_de_presencas_na_janela_por_dezena` | Total de vezes em que cada dezena (1 a 25) **saiu** nos concursos **desta janela** (contagem total no recorte; **equivalente** a `frequencia_por_dezena` no mesmo recorte). **Dica de leitura:** “total na janela” (não é sequência; não reinicia). |
| `sequencia_atual_de_presencas_por_dezena` | Comprimento da **sequência atual** de concursos em que cada dezena vem **saindo sem interrupção** no fim desta janela: quando a dezena **não sai** em um concurso, a sequência **reinicia** (volta a 0); quando volta a sair, começa a contar de novo. |
| `atraso_por_dezena` | Quantos **concursos** desde a **última** ocorrência da dezena até o fim do recorte considerado; 0 = saiu no **último** sorteio da janela. **Em linguagem leiga,** «ausência no sentido de *há N edições sem sair*» (vector com tipicamente 0, 1, 2, …) alinha a **este** nome, não a `frequencia_por_dezena` e não a `ausencia_blocos` (ver nota *Ausência*). |
| `estado_atual_dezena` | No **último** concurso da janela: `0` se a dezena saiu, senão o atraso corrente (ver catálogo). Complementa o painel quando se quer «situação de agora» alinhada ao mesmo *look* de vector que o atraso. |
...
| `top10_maiores_totais_de_presencas_na_janela` / `top10_menores_totais_de_presencas_na_janela` | Lista compacta das dezenas com maior (ou menor) **total de presenças na janela**, derivada do indicador “total_de_presencas_na_janela_por_dezena” no mesmo recorte. |

<a id="nota-ausencia-adr-0021"></a>

**Nota (vocabulário «ausência» / quatro *papéis*):** **(1) *Frequência na janela*** (quantas vezes a dezena *saiu* no recorte) → *export* **`QtdFrequencia`**, métrica canónica `frequencia_por_dezena` (contagens, ver [secção *QtdFrequencia* no catálogo](../metric-catalog.md#export-legado-qtdfrequencia)). **(2) *Atraso* / *estado* no fim de janela** (no leigo, «*ausente* há *N* concursos») → `atraso_por_dezena` e, para «situação *agora*», `estado_atual_dezena` — **não** é a coluna/vector de *frequencia_por_dezena*. **(3)** **`ausencia_blocos`**: listas de comprimentos de blocos *sem* a dezena (*shape* `count_list_by_dezena` na Tabela 1 do catálogo) — outro *layout* do que o vector 1..25 de atraso. **(4) *Distinção em tabela*:** a tabela *Vocabulário* no [glossário (âncora)](../metric-glossary.md#vocab-ausencia-adr-0021) e as linhas de tabela A acima fixam a leitura; não renomear métricas. Alinhamento de fundo: [Ponte *— quatro papéis* (catálogo)](../metric-catalog.md#export-legado-qtdfrequencia) e, para definição fechada, [Tabela 1](../metric-catalog.md#tabela-1-identificacao-e-tipagem) e [Tabela 2](../metric-catalog.md#tabela-2-semantica) do *metric-catalog*.
...
*Última revisão desta ADR:* alinhar com [metric-glossary.md](../metric-glossary.md) sempre que a definição canónica de uma métrica mudar (bump de versão de métrica / catálogo). **2026-04-24 (b):** tabela A e nota *Ausência* alinhadas ao [metric-catalog](../metric-catalog.md) e ao glossário — «ausência» leiga ≈ atraso (vector 25), distinto de frequência (somas) e de `ausencia_blocos`. **2026-04-24 (c):** ids estáveis de ligação `vocab-ausencia-adr-0021` (glossário), `export-legado-qtdfrequencia` e ponte *— quatro papéis* (catálogo *QtdFrequencia*), `apendice-frases-modelo-pt-adr-0021` e `nota-ausencia-adr-0021` (Apêndice); D3 enumera os mesmos *papéis* que a ponte.
