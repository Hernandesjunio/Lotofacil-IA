...
public sealed class EntropiaLinhaPorConcursoMetric
{
    public WindowMetricValue Compute(DrawWindow window)
    {
        ...
        if (window.Draws.Count != window.Size)
        {
            throw new DomainInvariantViolationException(
                "entropia_linha_por_concurso: draw count must match the resolved window size.");
        }
        ...
        return new WindowMetricValue(
            MetricName: "entropia_linha_por_concurso",
            Scope: "series",
            Shape: "series",
            Unit: "bits",
            Version: "1.0.0",
