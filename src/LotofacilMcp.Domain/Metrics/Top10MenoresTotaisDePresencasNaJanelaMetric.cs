...
public sealed class Top10MenoresTotaisDePresencasNaJanelaMetric
{
    private readonly TotalDePresencasNaJanelaPorDezenaMetric _totalByDezena;

    public Top10MenoresTotaisDePresencasNaJanelaMetric(TotalDePresencasNaJanelaPorDezenaMetric totalByDezena)
    {
    ...
    public WindowMetricValue Compute(DrawWindow window)
    {
        ...
        var top10 = Enumerable.Range(1, 25)
            .OrderBy(d => totals[d - 1])
            .ThenBy(d => d)
            .Take(10)
            .ToArray();

        return new WindowMetricValue(
            MetricName: "top10_menores_totais_de_presencas_na_janela",
            Scope: "window",
