...
public static class V0ToolEndpoints
{
    ...
    {
        MapToolRoute(endpoints, "/tools/discover_capabilities", HandleDiscoverCapabilitiesAsync);
        ...
        MapToolRoute(endpoints, "/mcp/tools/discover_capabilities", HandleDiscoverCapabilitiesAsync);
    ...
    private static async Task<IResult> HandleDiscoverCapabilitiesAsync(HttpRequest request, V0Tools tools)
    {
        var (toolRequest, bindingError) = await ToolRequestBinding.BindAsync<DiscoverCapabilitiesRequest>(request);
        if (bindingError is not null)
        {
            return Results.BadRequest(bindingError);
        }

        var response = tools.DiscoverCapabilities(toolRequest!);
        return response is ContractErrorEnvelope errorEnvelope
            ? Results.BadRequest(errorEnvelope)
            : Results.Ok(response);
    }
