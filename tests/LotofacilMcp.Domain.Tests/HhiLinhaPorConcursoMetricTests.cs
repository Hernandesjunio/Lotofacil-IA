...
public sealed class HhiLinhaPorConcursoMetricTests
{
    [Fact]
    public void Hhi_FromCounts_UniformFiveBins_IsOneFifth()
    {
        Assert.Equal(0.2d, HerfindahlHirschmanIndex.FromNonNegativeCounts([3, 3, 3, 3, 3]), 12);
    }

    [Fact]
    public void Hhi_FromCounts_AllMassInOneBin_IsOne()
    {
        Assert.Equal(1d, HerfindahlHirschmanIndex.FromNonNegativeCounts([15, 0, 0, 0, 0]), 12);
    }

    [Fact]
    public void Hhi_NegativeCount_Throws()
    {
        Assert.Throws<DomainInvariantViolationException>(() =>
            HerfindahlHirschmanIndex.FromNonNegativeCounts([16, -1, 0, 0, 0]));
    }

    [Fact]
    public void Serie_OnMinimalFixture_MatchesRowHistogramHhi()
    {
        var window = BuildWindowFromSyntheticFixture(endContestIdInclusive: 3);
        var sut = new HhiLinhaPorConcursoMetric();

        var metric = sut.Compute(window);

        Assert.Equal("hhi_linha_por_concurso", metric.MetricName);
        ...
        {
            VolanteRowColumnCounts.FillRowCounts(window.Draws[i], row);
            expected[i] = HerfindahlHirschmanIndex.FromNonNegativeCounts(row);
        }
    ...
    [Fact]
    public void Draw_OneToFifteen_ThreeFullRows_MatchesHhi()
    {
        ...
        var h = HerfindahlHirschmanIndex.FromNonNegativeCounts([5, 5, 5, 0, 0]);
        Assert.Equal(h, Assert.Single(metric.Value), 12);
    }

    [Fact]
    public void WindowMetricDispatcher_DispatchesHhiLinhaPorConcurso()
    {
        var window = BuildWindowFromSyntheticFixture(endContestIdInclusive: 3);
        var sut = WindowMetricDispatcherFactory.Create();

        var metric = sut.Dispatch("hhi_linha_por_concurso", window);
